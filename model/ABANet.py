...
class AttentionBlock(nn.Module):
    ...
    def forward(self, g, x):
        ...
        x1 = self.W_x(x)
        psi = self.relu(g1 + x1)
        # psi = self.relu(torch.concat((g1, x1), 1))
        psi = self.psi(psi)
        psi = self.upscore2(psi)
        return x * psi
class ABANet(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(ABANet, self).__init__()
        ...
        ## -------------Attention--------------
        self.att1 = AttentionBlock(F_g=14, F_l=14, F_int=7)
        self.att2 = AttentionBlock(F_g=28, F_l=28, F_int=14)
        self.att3 = AttentionBlock(F_g=512, F_l=256, F_int=512)
        self.att4 = AttentionBlock(F_g=256, F_l=128, F_int=256)
        self.att5 = AttentionBlock(F_g=128, F_l=64, F_int=128)
        ## -------------Decoder--------------
    ...
    def forward(self, x):
        ...
        hx_m3 = self.relu4d_m(self.bn4d_m(self.conv4d_m(hx)))
        #hd4 = self.relu4d_2(self.bn4d_2(self.conv4d_2(hx)))

        hx_att3 = self.att3(hx_m3, h3)

        #hx = self.upscore2(hd4)  # 32 -> 64


        hx = self.relu3d_1(self.bn3d_1(hx_att3))
        hx_m4 = self.relu3d_m(self.bn3d_m(self.conv3d_m(hx)))
        # hd3 = self.relu3d_2(self.bn3d_2(self.conv3d_2(hx)))
        hx_att4 = self.att4(hx_m4, h2)
        #hx = self.upscore2(hd3)  # 64 -> 128

        hx = self.relu2d_1(self.bn2d_1(hx_att4))
        hx_m5 = self.relu2d_m(self.bn2d_m(self.conv2d_m(hx)))

        # hd2 = self.relu2d_2(self.bn2d_2(self.conv2d_2(hx_m5)))

        # hx = self.upscore2(hd2)  # 128 -> 256

        hx_att5 = self.att5(hx_m5,h1)
        hx = self.relu1d_1(self.bn1d_1(hx_att5))
        ...
        d4 = self.outconv5(hx_m3)
        d4 = self.upscore4(d4)  # 32->256

        d3 = self.outconv4(hx_m4)
        d3 = self.upscore3(d3)  # 64->256

        d2 = self.outconv3(hx_m5)
